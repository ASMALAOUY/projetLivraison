...
// Composant carte chargé dynamiquement pour éviter erreurs SSR/import
function LiveMap({ driverId, destLat, destLng, destLabel }) {
  ...
  if (!MapReady || !MapComponents) return (
  ...
  const center = pos ? [pos.latitude, pos.longitude] : [destLat || 31.6295, destLng || -7.9811]
  ...
  return (
    <div>
      <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-50">
        ...
        {!pos && <span className="text-xs text-gray-400 ml-auto">En attente du livreur…</span>}
      </div>
      ...
      <p className="text-xs text-gray-400 text-center py-2">
        Position mise à jour toutes les 10 secondes
      </p>
    </div>
  )
}
...
export default function TrackingPage() {
  ...
  const CATALOGUE = [
    ...
    { id: 5, name: 'Pharmacie',           emoji: '💊', price: 12,  description: 'Médicaments' },
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        {step === 1 && (
          <div>
            ...
            <p className="text-sm text-gray-400 mb-5">Sélectionnez un ou plusieurs articles</p>
              ...
              {CATALOGUE.map(product => {
                ...
                return (
                  <div key={product.id}
                    ...
                    className={`bg-white rounded-2xl border-2 p-4 cursor-pointer transition ${qty > 0 ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-gray-200'}`}>
                    <div className="flex items-start justify-between mb-2">
                      <span className="text-2xl">{product.emoji}</span>
                      {qty > 0 && (
                        <div className="flex items-center gap-1 bg-blue-600 text-white rounded-full px-2 py-0.5">
                          <button onClick={e => { e.stopPropagation(); removeFromCart(product.id) }} className="text-sm font-bold">−</button>
                          <span className="text-xs font-bold">{qty}</span>
                          <button onClick={e => { e.stopPropagation(); addToCart(product) }} className="text-sm font-bold">+</button>
                        </div>
                      )}
                    </div>
            ...
            {cartCount > 0 && (
              <button onClick={() => setStep(2)}
                className="w-full bg-blue-600 text-white rounded-2xl py-4 font-bold flex items-center justify-between px-6">
        ...
        {step === 3 && (
          <div>
            <h1 className="text-xl font-bold text-gray-800 mb-5">Résumé</h1>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-4">
              <h3 className="text-sm font-bold text-gray-700 mb-3">Articles</h3>
              {cartItems.map(item => (
                <div key={item.id} className="flex justify-between mb-2">
                  <span className="text-sm">{item.emoji} {item.name} × {item.qty}</span>
                  <span className="text-sm font-bold text-blue-600">{item.qty * item.price} MAD</span>
                </div>
              ))}
            ...
            <button onClick={handleSubmit} disabled={submitting}
              ...
              {submitting ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>Envoi…</> : `Confirmer · ${cartTotal} MAD`}
            </button>
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
          <button onClick={() => setView('list')} className="text-sm text-gray-500 hover:text-gray-800">
            ← Mes commandes
          </button>
              ...
              <div className="flex items-center justify-between relative">
                ...
                {STEPS.map((step, i) => (
                  <div key={step.key} className="flex flex-col items-center gap-2 z-10">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg border-2 ${i <= stepIdx ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-200'}`}>
                      {step.emoji}
                    </div>
                    <span className={`text-xs font-semibold ${i <= stepIdx ? 'text-blue-600' : 'text-gray-300'}`}>{step.label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {items.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              <h2 className="text-sm font-bold text-gray-700 mb-3">Articles commandés</h2>
              {items.map((item, i) => (
              ...
              {selected.totalPrice && (
                <div className="border-t pt-2 flex justify-between mt-1">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3 text-sm">
            ...
            {selected.pickupAddress && <div className="flex gap-3"><span>📍</span><div><p className="text-xs text-gray-400">Récupéré chez</p><p className="font-medium">{selected.pickupAddress}</p></div></div>}
          </div>
                    ...
                    <button key={order.id}
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {order.status === 'in_progress' && (
                          <span className="text-xs text-blue-600 font-medium ml-1">🚴 En route</span>
                        )}
                      ...
                      <div className="flex gap-1.5 mt-3">
                        {STEPS.map((step, i) => (
                          <div key={step.key} className={`h-1.5 flex-1 rounded-full ${i <= stepIdx ? 'bg-blue-500' : 'bg-gray-100'}`}/>
                        ))}
                      </div>
                    </button>
                  )
                  ...
                  return (
                    <button key={order.id}
                      ...
                      <div className="flex gap-1">
                        {items.slice(0,3).map((item,i) => <span key={i} className="text-lg">{item.emoji}</span>)}
                      </div>
